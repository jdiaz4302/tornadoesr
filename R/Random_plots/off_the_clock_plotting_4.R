...
# Call the data
source("R/3_format_for_injuries_visualizations.R")
...
# Get rid of outliers that are messing up visualizations
tor_df <- dplyr::filter(tor_df,
                        INJURIES_DIRECT < 300 & INJURIES_DIRECT > 0)


# Get property damage per year
InjuriesPerState <- aggregate(tor_df$INJURIES_DIRECT,
...
InjuriesPerState$Category <- as.factor(InjuriesPerState$Category)


# Get the value for the state with the 10th most
# tornado-induced property damage
top_10_states_cutoff <- sort(InjuriesPerState$x, decreasing = TRUE)[10]


# Keep only the top 10 states
InjuriesPerState <- filter(InjuriesPerState,
                           x >= top_10_states_cutoff)


# Get those state names
state_names <- InjuriesPerState$Category
...
ggplot(tor_top_10_df,
  ...
  labs(x = "State",
       y = "Injuries",
       title = "Tornado-Induced Injuries",
       subtitle = "Per Event in the Top 10 Most Injured States") +
...
map_plot <- ggmap(US_map) +
  ...
  labs(x = "Longitude",
       y = "Latitude",
       title = "Tornado-Induced Injuries by Event",
       fill = "Injuries") +
...
map_gif <- ggmap(US_map) +
  ...
  labs(x = "Longitude",
       y = "Latitude",
       title = "Tornado-Induced Injuries by Event in:",
       fill = "Injuries") +
  theme(plot.title = element_text(hjust = 0.5, size = 27),
        axis.title = element_text(size = 20),
        axis.text = element_text(size = 18))
...
tor_df <- tor_df[with(tor_df, order(BEGIN_DATE_TIME, INJURIES_DIRECT)), ]

rownames(tor_df) <- NULL

tor_df$CUM_INJURIES <- zoo::rollapplyr(tor_df$INJURIES_DIRECT,
                                       length(tor_df$INJURIES_DIRECT),
                                       partial = TRUE,
                                       FUN = sum)
...
saveGIF( {
  for (i in 1:20) {
    print(ggplot(tor_df) +
            ...
            labs(x = "Date",
                 y = "Injuries",
                 title = "Cumulative Tornado-Induced Injuries since Jan 1, 1997") +
